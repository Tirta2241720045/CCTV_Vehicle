...
model.to('cpu')  # Pindahkan model ke GPU
...
def process_video(input_path, frame_width=1536, frame_height=864):
    ...
    try:
        ...
        while True:
            ...
            with torch.no_grad():  # Nonaktifkan autograd untuk CPU (tidak perlu di CUDA)
                results = model.track(frame, persist=True, device='cpu')  # Gunakan CPU
